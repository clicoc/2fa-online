...
function upd(){const b=D('tb');const r=30-(Math.floor(Date.now()/1000)%30);b.innerHTML="";for(let a of acts){const c=a.t.generate();const tr=document.createElement('tr');tr.innerHTML=\`<td>\${a.n}</td><td class="code" onclick="cp('\${c}')">\${c}</td><td class="remain">\${r}s</td>\`;b.appendChild(tr)}if(r===30)setTimeout(upd,500)}
